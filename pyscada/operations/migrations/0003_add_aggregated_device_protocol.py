    ...
    if not DeviceProtocol.objects.using(db_alias).filter(pk=PROTOCOL_ID):
        DeviceProtocol.objects.using(db_alias).bulk_create(
            [
                DeviceProtocol(
                    pk=PROTOCOL_ID,
                    protocol=aggretation_name.lower(),
                    description=f"{aggretation_name} Interface",
                    app_name=f"pyscada.{__app_name__.lower()}",
                    device_class=f"pyscada.{__app_name__.lower()}.device",
                    daq_daemon=True,
                    single_thread=True,
                ),
            ]
...
def reverse_func(apps, schema_editor):
    ...
    DeviceProtocol = apps.get_model("pyscada", "DeviceProtocol")
    db_alias = schema_editor.connection.alias
    DeviceProtocol.objects.using(db_alias).filter(
        protocol=aggretation_name.lower()
    ).delete()
